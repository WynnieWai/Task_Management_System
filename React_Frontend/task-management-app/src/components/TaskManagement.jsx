...
//   // Modify your useEffect for handling role changes
//   // useEffect(() => {
//   //   const handleRoleChange = async (event) => {
//   //     const { userId, newRole, oldRole } = event.detail;
//   //     await handleRoleChangeSideEffects(userId, newRole, oldRole);
...
//   //     const usersResponse = await axios.get("http://localhost:5047/api/User");
//   //     setAllUsers(usersResponse.data);
//   //     const locked = usersResponse.data.filter(u => u.status === "Locked").map(u => u.username);
//   //     setLockedUsernames(locked);

//   //     // Then fetch project details
//   //     const projectResponse = await axios.get(`http://localhost:5047/api/projects/${projectId}`);
//   //     setProjectStatus(projectResponse.data.status || "Active");
//   //     setProjectMembers(projectResponse.data.members || []);

//   //     // Finally fetch tasks with proper filtering
//   //     const tasksResponse = await getTasksByProject(projectId);
//   //     const updatedTasks = tasksResponse.data.map(task => {
//   //       // Filter out any non-contributors from task members
//   //       const updatedMembers = task.members?.filter(member => {
//   //         const user = usersResponse.data.find(u => u.username === member);
//   //         return user?.role?.toLowerCase() === 'contributor';
//   //       }) || [];
...
//   //   } catch (err) {
//   //     alert("Failed to load data: " + (err?.response?.data || err));
//   //   }
//   // };
...
//       // Filter project members to only include contributors
//       const allMembers = (projectResponse.data.members || "").split(',')
//         .map(m => m.trim())
//         .filter(m => m);
      
//       const contributorMembers = allMembers.filter(member => {
//         const user = usersResponse.data.find(u => u.username === member);
//         return user?.role?.toLowerCase() === 'contributor';
//       });
//       setProjectMembers(contributorMembers);

//       // Finally fetch tasks with proper filtering
//       const tasksResponse = await getTasksByProject(projectId);
//       const updatedTasks = tasksResponse.data.map(task => {
//         // Filter out any non-contributors from task members
//         const taskMembers = (task.members || "").split(',')
//           .map(m => m.trim())
//           .filter(m => m);
        
//         const updatedMembers = taskMembers.filter(member => {
//           const user = usersResponse.data.find(u => u.username === member);
//           return user?.role?.toLowerCase() === 'contributor';
//         });
...
//   // Enhance your role change side effects handler
...
//         prevTasks.map(task => {
//           const taskMembers = task.members?.split(',') || [];
//           if (taskMembers.includes(user.username)) {
//             return {
//               ...task,
//               members: taskMembers.filter(m => m !== user.username).join(',')
//             };
//           }
//           return task;
//         })
...
//       // Also update project members if needed
//       setProjectMembers(prevMembers => 
//         prevMembers.filter(m => m !== user.username)
//       );
//     }
//   } catch (err) {
...
//       await axios.put(`http://localhost:5047/api/User/update-role/${userId}`, {
//         newRole
//       });
...
//         detail: { userId, newRole, oldRole }
//       }));

//       // Show success message
//       addNotification({
//         message: `Role changed to ${newRole} for ${user.username}`
//       });
//     } catch (err) {
//       alert("Failed to change role: " + (err?.response?.data || err));
//     }
//   };
...
//                         {projectMembers.map((m) => {
//                           const isLocked = lockedUsernames.includes(m);
//                           const currentMembers = (editTask.members || "").split(',')
//                             .map(mem => mem.trim())
//                             .filter(mem => mem);
//                           const isChecked = currentMembers.includes(m);
...
export default function TaskManagement({ user }) {
  ...
  useEffect(() => {
    const handleUserRoleChanged = () => {
      ...
      getTasksByProject(projectId)
        .then((res) => setTasks(res.data))
        .catch((err) => alert("Failed to reload tasks after user role change: " + err));
    };
    window.addEventListener('userRoleChanged', handleUserRoleChanged);
    return () => window.removeEventListener('userRoleChanged', handleUserRoleChanged);
  }, [projectId]);
  ...
  const handleStartDateChange = (e) => {
    ...
    setEditTask(prev => {
      ...
      // If new start date is after current due date, clear due date
      const newDueDate = prev.dueDate && new Date(newStartDate) > new Date(prev.dueDate) 
        ? "" 
        : prev.dueDate;
  ...
  const handleSave = (idx) => {
    ...
    // Date validation
    if (!validateDates(editTask.startDate, editTask.dueDate)) {
      return alert("Due date must be after start date");
    }
                  ...
                  <td className="border px-4 py-2 text-sm font-normal">
                    {(canEdit && editIdx === idx) ? (
                      <div className="flex gap-2 items-center">
                        <div className={`relative ${
                          !editTask.startDate || !editTask.dueDate || validateDates(editTask.startDate, editTask.dueDate) 
                            ? '' 
                        ...
                        <div className={`relative ${
                          ...
                          <input
                            ...
                            className="border px-2 py-1 rounded w-full text-sm font-normal"
                            min={editTask.startDate || new Date().toISOString().split('T')[0]}
                          />
                          {(!editTask.dueDate || !validateDates(editTask.startDate, editTask.dueDate)) && (
                            <span className="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500">*</span>
                          )}
                        </div>
                      </div>
                    ) : (
                      `${task.startDate?.substring(0, 10) ?? ''} - ${task.dueDate?.substring(0, 10) ?? ''}`
                    )}
                    
                    {/* Error message positioned below the date inputs */}
                    {canEdit && editIdx === idx && !validateDates(editTask.startDate, editTask.dueDate) && (
                      <p className="text-red-500 text-xs mt-1">Due date must be after start date</p>
                    )}
                  </td>
                  <td className="border px-4 py-2 text-sm font-normal">
                    {renderDueStatusBadge(task)}
